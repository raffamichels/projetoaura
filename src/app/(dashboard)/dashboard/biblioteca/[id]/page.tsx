...
import { BookOpen, FilmSlate, CaretLeft, FloppyDisk, Trash, PencilSimple, X, Quotes, Calendar, Clock, Globe, Sparkle, ShareNetwork } from '@phosphor-icons/react';
...
export default function DetalheMidiaPage() {
  ...
  const getStatusBadge = (status: StatusLeitura) => {
    const badges = {
      PROXIMO: { label: 'Próximo', className: 'text-ink-faint' },
    ...
    <div className="space-y-4 sm:space-y-6 p-4 lg:p-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <Button
          ...
          className="bg-surface border border-line hover:bg-surface-hover text-ink-soft hover:text-ink w-full sm:w-auto"
        >
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            className="bg-brand hover:bg-brand-dark flex-1 sm:flex-none text-sm"
          >
            <ShareNetwork className="w-4 h-4 sm:mr-2" />
          ...
          {!editando ? (
            <>
              ...
              <Button
                ...
                className="bg-brand hover:bg-brand-dark flex-1 sm:flex-none text-sm"
              >
                <PencilSimple className="w-4 h-4 sm:mr-2" />
                <span className="hidden sm:inline">Editar</span>
              </Button>
              <Button
                variant="default"
                onClick={handleExcluir}
                className="border-line text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-500/10 px-3 sm:px-4"
              >
            ...
            <>
              <Button
                ...
                className="bg-surface border border-line text-ink-soft hover:bg-surface-hover flex-1 sm:flex-none text-sm"
              >
            ...
            <div className="shrink-0">
              {midia.capa ? (
                <img
                  ...
                  className="w-48 h-72 sm:w-56 sm:h-84 lg:w-64 lg:h-96 object-cover rounded-xl border border-line-strong shadow-lg mx-auto lg:mx-0"
                />
              ) : (
                <div className="w-48 h-72 sm:w-56 sm:h-84 lg:w-64 lg:h-96 bg-surface-hover rounded-xl border border-line flex items-center justify-center mx-auto lg:mx-0">
                  {midia.tipo === 'LIVRO' ? (
                    <BookOpen className="w-16 h-16 sm:w-20 sm:h-20 text-ink-faint" />
                  ) : (
                    <FilmSlate className="w-16 h-16 sm:w-20 sm:h-20 text-ink-faint" />
                  )}
                </div>
              )}
            </div>
            ...
            <div className="flex-1 space-y-4 sm:space-y-6">
              ...
              <div>
                <div className="flex items-center gap-2 sm:gap-3 mb-2">
                  {midia.tipo === 'LIVRO' ? (
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-brand-soft flex items-center justify-center">
                      <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-brand-dark" />
                    </div>
                  ) : (
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-surface-hover flex items-center justify-center">
                      <FilmSlate className="w-4 h-4 sm:w-5 sm:h-5 text-ink-soft" />
                    </div>
                  )}
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                {editando ? (
                  <select
                    ...
                    className="px-3 py-1.5 rounded-full bg-surface-hover border border-line-strong text-xs sm:text-sm font-medium text-ink focus:outline-none focus:ring-2 focus:ring-brand"
                  >
                  ...
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-hover border border-line-strong">
                    <div className={`w-2 h-2 rounded-full ${
                      midia.status === 'CONCLUIDO' ? 'bg-green-600' :
                      midia.status === 'EM_ANDAMENTO' ? 'bg-brand' :
                      midia.status === 'PAUSADO' ? 'bg-gold' : 'bg-ink-faint'
                    }`} />
              ...
              {/* Detalhes em Grid */}
              <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 pt-4 border-t border-line">
                {midia.genero && (
                  <div className="space-y-1">
                    <p className="text-xs text-ink-faint uppercase tracking-wider">Gênero</p>
                    <p className="text-ink font-medium">{midia.genero}</p>
                  </div>
                )}
                ...
                {midia.tipo === 'LIVRO' && (
                  <>
                    {midia.editora && (
                      <div className="space-y-1">
                        <p className="text-xs text-ink-faint uppercase tracking-wider">Editora</p>
                        <p className="text-ink font-medium">{midia.editora}</p>
                      </div>
                    )}
                    {midia.fonte && (
                      <div className="space-y-1">
                        <p className="text-xs text-ink-faint uppercase tracking-wider">Fonte</p>
                        <p className="text-ink font-medium">{midia.fonte}</p>
                      </div>
                    )}
                  </>
                )}
                    ...
                    {midia.anoLancamento && (
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-xs text-ink-faint uppercase tracking-wider">Ano</p>
                        </div>
                        <p className="text-ink font-medium">{midia.anoLancamento}</p>
                      </div>
                    )}
                    {midia.duracao && (
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-xs text-ink-faint uppercase tracking-wider">Duração</p>
                        </div>
                        <p className="text-ink font-medium">{midia.duracao} min</p>
                      </div>
                    )}
                  </>
                )}
                ...
                {midia.idioma && (
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs text-ink-faint uppercase tracking-wider">Idioma</p>
                    </div>
                    <p className="text-ink font-medium">{midia.idioma}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {midia.resenhaGeradaIA && (
        <Card className="bg-surface border-line">
          <CardHeader className="flex flex-row items-center justify-between p-4 sm:p-6">
            <CardTitle className="text-sm sm:text-base md:text-lg text-ink flex items-center gap-2">
              <Sparkle className="w-4 h-4 sm:w-5 sm:h-5 text-gold" />
          ...
          <CardContent className="p-4 sm:p-6 pt-0">
            {editando ? (
              <Textarea
                ...
                className="bg-surface border-line-strong text-ink min-h-50 font-serif text-sm sm:text-base"
                placeholder="Resenha gerada pela IA..."
              />
            ) : (
              <p className="text-ink-soft whitespace-pre-wrap text-sm sm:text-base font-serif leading-relaxed">
                {midia.resenhaGeradaIA || (
                  <span className="text-ink-faint italic text-xs sm:text-sm">Nenhuma resenha gerada ainda.</span>
                )}
              </p>
            )}
          </CardContent>
      ...
      <div className="space-y-2">
        <Card className="bg-surface border-line">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium text-ink-soft uppercase tracking-wider">Impressões Iniciais</span>
            </div>
            {editando ? (
              <Textarea
                ...
                className="bg-surface border-line-strong text-ink min-h-16 text-sm resize-none"
                placeholder="Suas primeiras impressões..."
              />
            ) : (
              <p className="text-ink-soft whitespace-pre-wrap text-sm">
                {midia.impressoesIniciais || (
                  <span className="text-ink-faint italic text-xs">Nenhuma impressão registrada.</span>
                )}
              </p>
            )}
          </CardContent>
        ...
        <Card className="bg-surface border-line">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium text-ink-soft uppercase tracking-wider">Principais Aprendizados</span>
            </div>
            {editando ? (
              <Textarea
              ...
              <p className="text-ink-soft whitespace-pre-wrap text-sm">
                {midia.principaisAprendizados || (
                  <span className="text-ink-faint italic text-xs">Nenhum aprendizado registrado.</span>
                )}
              </p>
            )}
          </CardContent>
        ...
        <Card className="bg-surface border-line">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium text-ink-soft uppercase tracking-wider">Trechos Memoráveis</span>
            </div>
            {editando ? (
              <Textarea
              ...
              <p className="text-ink-soft whitespace-pre-wrap text-sm">
                {midia.trechosMemoraveis || (
                  <span className="text-ink-faint italic text-xs">Nenhum trecho registrado.</span>
                )}
              </p>
            )}
          </CardContent>
        ...
        <Card className="bg-surface border-line">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium text-ink-soft uppercase tracking-wider">Reflexão</span>
            </div>
            {editando ? (
              <Textarea
              ...
              <p className="text-ink-soft whitespace-pre-wrap text-sm">
                {midia.reflexao || (
                  <span className="text-ink-faint italic text-xs">Nenhuma reflexão registrada.</span>
                )}
              </p>
            )}
          </CardContent>
        ...
        <Card className="bg-surface border-line">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium text-ink-soft uppercase tracking-wider">Aprendizados Práticos</span>
            </div>
            {editando ? (
              <Textarea
              ...
              <p className="text-ink-soft whitespace-pre-wrap text-sm">
                {midia.aprendizadosPraticos || (
                  <span className="text-ink-faint italic text-xs">Nenhum aprendizado prático registrado.</span>
                )}
              </p>
            )}
          </CardContent>
        ...
        <Card className="bg-surface border-line">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium text-ink-soft uppercase tracking-wider">Considerações Finais</span>
            </div>
            {editando ? (
              <Textarea
              ...
              <p className="text-ink-soft whitespace-pre-wrap text-sm">
                {midia.consideracoesFinais || (
                  <span className="text-ink-faint italic text-xs">Nenhuma consideração final registrada.</span>
                )}
              </p>
            )}
          </CardContent>
      ...
      {midia.citacoes && midia.citacoes.length > 0 && (
        <Card className="bg-surface border-line">
          <CardHeader className="p-4 sm:p-6">
            <CardTitle className="flex items-center gap-2 text-ink text-sm sm:text-base md:text-lg">
              <Quotes className="w-4 h-4 sm:w-5 sm:h-5 text-gold" />
          ...
          <CardContent className="space-y-3 p-4 sm:p-6 pt-0">
            {midia.citacoes.map((citacao) => (
              <div
                key={citacao.id}
                className="border-l-4 border-brand pl-3 sm:pl-4 py-2 bg-surface-hover rounded"
              >
                <p className="text-ink italic text-xs sm:text-sm md:text-base">&ldquo;{citacao.texto}&rdquo;</p>
                {citacao.pagina && (
                  <p className="text-xs text-ink-faint mt-1">Página {citacao.pagina}</p>
                )}
              </div>
            ))}
          </CardContent>
