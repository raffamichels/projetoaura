...
import { Download, Spinner, ShareNetwork, X, DeviceMobile, SquaresFour } from '@phosphor-icons/react'
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="bg-surface border-line text-ink sm:max-w-xl w-[95vw] max-h-[95vh] overflow-y-auto flex flex-col p-4 sm:p-6">
          <DialogHeader className="space-y-3 sm:space-y-4">
            <div>
              <DialogTitle className="flex items-center gap-2 text-base sm:text-lg">
                <ShareNetwork className="w-4 h-4 sm:w-5 sm:h-5 text-brand-dark" />
                Compartilhar
              </DialogTitle>
            ...
            <div className="flex p-1 bg-surface-hover rounded-lg border border-line">
              <button
              ...
              >
                <DeviceMobile className="w-3 h-3 sm:w-4 sm:h-4" />
          ...
          {/* RODAPÉ */}
          <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center gap-2 sm:justify-between mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-line">
            <Button
              variant="ghost"
              onClick={onClose}
              className="text-ink-soft hover:text-ink hover:bg-surface-hover w-full sm:w-auto text-sm"
            >
            ...
            <Button
              ...
              disabled={generating}
              className="bg-brand hover:bg-brand-dark w-full sm:w-auto sm:min-w-[140px] text-sm"
            >
              {generating ? (
                <>
